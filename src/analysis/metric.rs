...
pub trait Seminorm<K: UnitalRing, X: RingModule<K>, R: Real> {
    ...
    where
        K: From<R>,
    {
        x.clone() * K::from(self.norm(x).inv())
    }
}
...
///
pub trait InnerProduct<K: ComplexRing, M: RingModule<K>>:
    HermitianForm<K, M> + Norm<K, M, K::Real>
{
}

///A metric that also defines a norm
pub trait NormedMetric<K, X, R> = Norm<K, X, R> + Metric<X, R>
where
    K: UnitalRing,
    X: RingModule<K>,
    R: Real;
...
pub trait InnerProductSpace<F: ComplexRing>: RingModule<F> {
    ...
    fn norm_sqrd(self) -> F::Real {
        self.clone().inner_product(self).as_real()
    }
    ...
    fn orthogonal(self, rhs: Self) -> bool {
        self.inner_product(rhs).is_zero()
    }

    ///Computes the orthogonal component of `rhs` to `self`. equivalent to `rhs - self.project(rhs)`
    #[inline]
    fn reject(self, rhs: Self) -> Self
    where
        F: ComplexField,
    {
        rhs.clone() - self.project(rhs)
    }
